...
export default class PlayerLog extends React.Component {
  ...
  */
  constructor(props) {
    ...
    props.courses.forEach(course => {
      if (labels.includes(course.course_id)) {
        dataWeek[course.course_id].timeSpent = dataWeek[course.course_id].timeSpent + this.sumHours(course.time_spent, this.props.startRange ? this.props.startRange : startOfWeek,
          this.props.endRange ? this.props.endRange : moment())
      }
    })
    for (const [courseId, val] of Object.entries(dataWeek)) {
      dataWeek[courseId].width = val.timeSpent ? val.timeSpent > val.timeGoal ? maxWidth : maxWidth / val.timeGoal * val.timeSpent : 5
    }
    data.push(dataWeek)
    ...
    if (!this.props.startRange && !this.props.endRange) {
      do {
        ...
        props.courses.forEach(course => {
          if (labels.includes(course.course_id)) {
            dataWeek[course.course_id].timeSpent = dataWeek[course.course_id].timeSpent + this.sumHours(course.time_spent, weekStart, weekEnd)
          }
        })
        for (const [courseId, val] of Object.entries(dataWeek)) {
          dataWeek[courseId].width = val.timeSpent ? val.timeSpent > val.timeGoal ? maxWidth : maxWidth / val.timeGoal * val.timeSpent : 5
        ...
        if (keepGoing) { data.push(dataWeek) }
        index += 1
  ...
  sumHours(timeSpent, startRange, endRange) {
    ...
    timeSpent.forEach(element => {
      if (moment(element.started_at).isSameOrAfter(startRange, 'day') && moment(element.ended_at).isSameOrBefore(endRange, 'day')) {
        sum += moment(element.ended_at).diff(moment(element.started_at), 'hours')
      }
    })
    return sum
  }
  ...
  startAnimation() {
    ...
    Animated.parallel(labels.map(label => {
      return timing(anims[label], { toValue: data[label].width, useNativeDriver: false })
    })).start()
  }
  ...
  handleAnimation(newIndex) {
    ...
    const timing = Animated.timing
    Animated.parallel(labels.map(label => {
      return timing(anims[label], { toValue: newData[label].width, useNativeDriver: false })
    })).start(() => {this.setState(state)})  // callback triggers React lifecycle
  }
        ...
        {len > 0 &&
          <View style={styles.item}>
            ...
            <View style={styles.data}>
              ...
              <Animated.View style={[styles.bar, styles.color1, { width: anims[labels[0]] }]} />
              <Text style={styles.dataNumber}>{data[labels[0]].timeSpent}/{data[labels[0]].timeGoal} hours
              {data[labels[0]].timeSpent >= data[labels[0]].timeGoal ? "*" : ""}</Text>
            </View>
          </View>
        }
        {len > 1 &&
          <View style={styles.item}>
            ...
            <View style={styles.data}>
              <View style={[styles.barOverlay, { width: maxWidth }]} />
              <Animated.View style={[styles.bar, styles.color2, { width: anims[labels[1]] }]} />
              <Text style={styles.dataNumber}>{data[labels[1]].timeSpent}/{data[labels[1]].timeGoal} hours{data[labels[1]].timeSpent >= data[labels[1]].timeGoal ? "*" : ""}</Text>
            </View>
          </View>
        }
        {len > 2 &&
          <View style={styles.item}>
            ...
            <View style={styles.data}>
              <View style={[styles.barOverlay, { width: maxWidth }]} />
              <Animated.View style={[styles.bar, styles.color3, { width: anims[labels[2]] }]} />
              <Text style={styles.dataNumber}>{data[labels[2]].timeSpent}/{data[labels[2]].timeGoal} hours{data[labels[2]].timeSpent >= data[labels[2]].timeGoal ? "*" : ""}</Text>
            </View>
          </View>
        }
        {len > 3 &&
          <View style={styles.item}>
            ...
            <View style={styles.data}>
              <View style={[styles.barOverlay, { width: maxWidth }]} />
              <Animated.View style={[styles.bar, styles.color4, { width: anims[labels[3]] }]} />
              <Text style={styles.dataNumber}>{data[labels[3]].timeSpent}/{data[labels[3]].timeGoal} hours{data[labels[3]].timeSpent >= data[labels[3]].timeGoal ? "*" : ""}</Text>
            </View>
          </View>
        }
        {len > 4 &&
          <View style={styles.item}>
            ...
            <View style={styles.data}>
              <View style={[styles.barOverlay, { width: maxWidth }]} />
              <Animated.View style={[styles.bar, styles.color5, { width: anims[labels[4]] }]} />
              <Text style={styles.dataNumber}>{data[labels[4]].timeSpent}/{data[labels[4]].timeGoal} hours{data[labels[4]].timeSpent >= data[labels[4]].timeGoal ? "*" : ""}</Text>
            </View>
          </View>
        }
        {len > 5 &&
          <View style={styles.item}>
            ...
            <View style={styles.data}>
              <View style={[styles.barOverlay, { width: maxWidth }]} />
              <Animated.View style={[styles.bar, styles.color6, { width: anims[labels[5]] }]} />
              <Text style={styles.dataNumber}>{data[labels[5]].timeSpent}/{data[labels[5]].timeGoal} hours{data[labels[5]].timeSpent >= data[labels[5]].timeGoal ? "*" : ""}</Text>
            </View>
          </View>
        }
        {len > 6 &&
          <View style={styles.item}>
            ...
            <View style={styles.data}>
              <View style={[styles.barOverlay, { width: maxWidth }]} />
              <Animated.View style={[styles.bar, styles.color7, { width: anims[labels[6]] }]} />
              <Text style={styles.dataNumber}>{data[labels[6]].timeSpent}/{data[labels[6]].timeGoal} hours{data[labels[6]].timeSpent >= data[labels[6]].timeGoal ? "*" : ""}</Text>
            </View>
          </View>
        }
        {len > 7 &&
          <View style={styles.item}>
            ...
            <View style={styles.data}>
              <View style={[styles.barOverlay, { width: maxWidth }]} />
              <Animated.View style={[styles.bar, styles.color1, { width: anims[labels[7]] }]} />
              <Text style={styles.dataNumber}>{data[labels[7]].timeSpent}/{data[labels[7]].timeGoal} hours{data[labels[7]].timeSpent >= data[labels[7]].timeGoal ? "*" : ""}</Text>
            </View>
          </View>
        }
        {len > 8 &&
          <View style={styles.item}>
            ...
            <View style={styles.data}>
              <View style={[styles.barOverlay, { width: maxWidth }]} />
              <Animated.View style={[styles.bar, styles.color2, { width: anims[labels[8]] }]} />
              <Text style={styles.dataNumber}>{data[labels[8]].timeSpent}/{data[labels[8]].timeGoal} hours{data[labels[8]].timeSpent >= data[labels[8]].timeGoal ? "*" : ""}</Text>
            </View>
          </View>
        }
      </View>
